...
from cryptography import Cryptography as CryptWindow
...
class ContainerWindow:
    ...
    def initUI(self):
        ...
        self.filemenu.add_command(
            label="New", command=lambda: self.createWindow("main", "", "")
        )
        self.filemenu.add_separator()
        self.filemenu.add_command(label="Exit", command=root.quit)
        self.menubar.add_cascade(
            label="File", menu=self.filemenu
        )  # Attached to a parent window instead of a toplevel window
        ...
        self.cryptographyMenu.add_command(
            label="Hash", command=lambda: self.createWindow("hash", "", "")
        )
        self.cryptographyMenu.add_command(
            label="Encryption", command=lambda: self.createWindow("hash", "", "")
        )
        ...
        # CREATE TABBAR
        self.tabBar = Frame(
            root, borderwidth=1, relief="raised", bg=self.settingsObject["tabBarColour"]
        )
    ...
    def createWindow(self, windowTitle, frameName, frameInfoItem):
        if windowTitle == "main":
            ...
            self.windowMenu.add_command(
                label=frameName, command=lambda: self.show_frame(frameName)
            )
    ...
    def removeFrame(self, frameName):
        ...
        if self.frameInfo[frameName]["path"] == "":
            if messagebox.askokcancel(
                "Quit",
                "Are you sure you want to quit, file:{} is not saved".format(frameName),
            ):
                pass
            else:
                removeFrame = False
    ...
    def createTab(self, frameName):
        ...
        QuitTabButton = Button(
            tabFrame, bg="red", command=lambda: self.removeFrame(frameName)
        )
    ...
    def changeActiveTab(self, frameName):
        self.tabs[self.currentFrame].winfo_children()[0].configure(
            bg=self.settingsObject["tabColour"]
        )
        self.tabs[frameName].winfo_children()[0].configure(
            bg=self.settingsObject["activeTabColour"]
        )

    # MANAGE FILE OPERATIONS-------------
    def openFile(self):
        filePath = filedialog.askopenfilename(
            initialdir=self.currentInstanceObject["lastPathUsed"],
            title="Select file",
    ...
    def saveAsFile(self):
        ...
        if self.frameInfo[self.currentFrame]["windowType"] == "main":
            currentFrameText = (
                self.frames[self.currentFrame].winfo_children()[1].get(1.0, END)
            )
    ...
    def changeFrameName(self, frameName):
        ...
        self.tabs[frameName].winfo_children()[0].bind(
            "<Button-1>", lambda e: self.show_frame(frameName)
        )
        ...
        if self.settingsObject["askUnsavedTabsOnClose"] == "CheckEach":
            for frameName in self.frames:
                ...
                if self.frameInfo[frameName]["path"] == "":
                    ...
                    if (
                        len(self.frames[frameName].winfo_children()[1].get(1.0, END))
                        > 1
                    ):
    ...
    def checkUnsavedTabsDialog(self, unsavedFrames):
        ...
        self.SFDlistBox = Listbox(
            saveFileDialog,
            listvariable=self.SFDoptions,
            exportselection=0,
            selectmode=MULTIPLE,
        ...
        self.saveNone = Button(
            saveFileDialog,
            text="Just delete all files",
            bg="red",
        ...
        for frameName in oldFrameInfo.keys():
            ...
            if os.path.isfile(oldFrameInfo[frameName]["path"]):
                ...
                if messagebox.askyesno(
                    "Load last session",
                    "The file {} does not exist anymore or has been moved, keep it open?".format(
                        frameName
                    ),
                ):
                    self.createWindow("main", frameName, oldFrameInfo[frameName])
...
class MainWindow(Frame):
    def __init__(self, parent, controller):
        ...
        self.verticalScrollbar = Scrollbar(
            parent, orient=VERTICAL, command=self.mainTextBox.yview
        )
        ...
        self.horizontalScrollbar = Scrollbar(
            parent, orient=HORIZONTAL, command=self.mainTextBox.xview
        )
